...
candidate_score_weights = {
    ...
    'language_score': 0.0,  # IMPLEMENTED but NOT adopted: gold NDCG@10 flat & reverse floors break at any w>0 (language coverage only 21% -> too sparse); revisit when coverage/gold grades language
    ...
    'attrition_score': 0.0,  # C5 P2: tenure/attrition (flight-risk from median completed-permanent tenure). Product-motivated, JD-independent. Held at 0 pending --ablate gold NDCG@10 sign-off.
    'experience_relevance_score': 0.0,  # C5 P3: relevant-vs-adjacent HR years (title-classified). Held at 0 pending --ablate + --redundancy(similarity, experience).
    'education_relevance_score': 0.03,  # C5 P4 ADOPTED (product-motivated, eval-neutral, 2026-07-29): soft HR/business-degree + CIPD/CHRP tie-breaker in [0.5,1.0] (never a gate). corr(judge)+0.21; gold NDCG@10 holds (.9512->.9505 within n=1 noise) at 0.03; mirrors location_score (real recruiter tie-breaker, CIPD is a JD nice-to-have), NOT an eval-proven gain -> floors NOT ratcheted.
}
