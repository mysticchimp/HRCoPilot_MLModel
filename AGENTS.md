...
- **LLM providers & models**: `core.llm.get_provider(name)` — **default provider is `anthropic`** (via `$LLM_PROVIDER` / `ANTHROPIC_API_KEY`, model `claude-sonnet-4-6`); pass `"copilot"` for GitHub Copilot (default model `claude-sonnet-4.5`). Pinned roles: reverse-match JD generation → `claude-opus-4.7` (`evals/cases.py:JDGEN_MODEL`, `run_eval.py --model`); blind judges → `claude-opus-4.8` + `gpt-5.5`; bench judge → `claude-opus-4.8`. `process_jd` / email generation use the provider default (no pinned model). Scoring HTTP service: `uvicorn api.main:app` (`POST /score`).
